...
### Misc:
- All data fetching is delayed by half a second, hardcoded in `src/app/test-data/test-data.service.ts`. Error handling is skipped for simplicity as it would entail adding user notification UI.
...
- As far as mobile layout goes, I just don't think phones are good for looking at tables. I made sure that interactive components are clearly visible, the table itself is just scrollable.
### Search:
